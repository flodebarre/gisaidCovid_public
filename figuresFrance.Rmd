...
# Check that we have all names
rr <- unique(dat$region)
rr <- rr[!is.na(rr)]
...
```{r cleanDataLocations, echo = FALSE, results = 'hide', eval = FALSE}
## Old version -- kept for legacy, but not used anymore
...
length(is.short)
length(missing.day)
aggregate(subIHU$Virus.name, by = list(Sanger = is.short, missingDay = missing.day), FUN = length)
...
table(subIHU.mostrecent$Submission.date.YMD)

table(subIHU.mostrecent$Collection.date.YMD)
...
subIHU.mostrecent[10,]
unique(subIHU$Variant)

unique(subIHU.mostrecent$Location)
nrow(subIHU.mostrecent)

subIHU.mostrecent[1,]

sort(table(subIHU.mostrecent$Pango.lineage), decreasing = TRUE)

byLin <- aggregate(subIHU.mostrecent$Pango.lineage, by = list(Lineage = subIHU.mostrecent$Pango.lineage, Collection.date = subIHU.mostrecent$Collection.date.YMD), FUN = length)
totdate <- aggregate(subIHU.mostrecent$Pango.lineage, by = list(Collection.date = subIHU.mostrecent$Collection.date.YMD), FUN = length)
names(totdate)[2] <- "tot"
...
byVar <- aggregate(subIHU.mostrecent$VariantShort, by = list(Variant = subIHU.mostrecent$VariantShort, Collection.date = subIHU.mostrecent$Collection.date.YMD), FUN = length)
totdate <- aggregate(subIHU.mostrecent$VariantShort, by = list(Collection.date = subIHU.mostrecent$Collection.date.YMD), FUN = length)
names(totdate)[2] <- "tot"
...
```{r}
par(mfrow = c(1, 1))
barplot(byL$x ~ byL$Lineage + byL$Collection.date, border = gray(0, 0), 
             xlab = "Collection date", ylab = "Nb sequences", 
             legend = TRUE, args.legend = list(x = "right", cex = 0.7, pt.cex = 0.9, bty = "n"), col = rep(brewer.pal(9, "Set1"), 10))
