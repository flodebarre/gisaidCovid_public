...
par(las = 1, mar = c(4, 4, 3, 4), 
    ...
    xpd = FALSE)
plot(Beta.byweek$week, Beta.byweek$p, xlim = c(19, max(Beta.byweek$week)) + c(-1, 0), 
     xlab = "Collection week (* = Flash week)", ylab = "Proportion Beta", ylim = c(0, 0.25), 
     frame.plot = FALSE, yaxs = "i", col = cols[1], pch = 16, main = "Beta")
axis(4)
...
arrows(x0 = Beta.byweek$week, y0 = Beta.byweek$p + Beta.byweek$deltaCI, 
       ...
       code = 3, angle = 90, length = 0.1, col = cols[1])

points(tessyBeta.byweek$week, tessyBeta.byweek$p, col = cols[3], pch = 15)

arrows(x0 = as.numeric(tessyBeta.byweek$week), y0 = tessyBeta.byweek$p + tessyBeta.byweek$deltaCI, 
       x1 = as.numeric(tessyBeta.byweek$week), y1 = tessyBeta.byweek$p - tessyBeta.byweek$deltaCI, 
       code = 3, angle = 90, length = 0.1, col = cols[3])

par(xpd = TRUE)
text(datesFlash[datesFlash$week >=18, "week"], 0.0075, "*")

legend("topleft", col = rev(cols[c(1, 3)]), legend = rev(c("in GISAID with country = France", "in TESSy")), pch = rev(c(16, 15)), bty = "n")
...
plot(L452R.byweek$week, L452R.byweek$p, xlim = range(dat.France$week, na.rm = TRUE) + c(-1, 0), 
     ...
     frame.plot = FALSE, yaxs = "i", col = cols[1], pch = 16, main = "L452R")
axis(4)
...
points(dat.France$week, dat.France$tx_C1/100, col = cols[2], pch = 16)
...
arrows(x0 = dat.France$week, y0 = dat.France$tx_C1/100 + dat.France$deltaCI_C1, 
       ...
       code = 3, angle = 90, length = 0.1, col = cols[2])

legend("topleft", col = cols[1:2], legend = c("Parmi les séquences sur GISAID", "Données de criblage"), pch = 16, bty = "n")
...
plot(E484K.byweek$week, E484K.byweek$p, xlim = range(dat.France$week, na.rm = TRUE) + c(-1, 0), 
     ...
     frame.plot = FALSE, yaxs = "i", col = cols[1], pch = 16, main = "E484K")
axis(4)
...
points(dat.France$week, dat.France$tx_A1/100, col = cols[2], pch = 16)
