...
rbind(tmp, prop_incomplete_collectionDate = round(tmp[1, ]/tmp[2, ], 4))
```
...
mtext(side = 1, "@flodebarre, données Gisaid", line = 3.5, adj = 0, cex = 0.6, col = gray(0.5), las = 1)
```
...
plot(L452R.byweek$week, L452R.byweek$p, xlim = range(dat.France$week, na.rm = TRUE) + c(-1, 0), 
     xlab = "Semaine de collecte", ylab = "Proportion Delta ou L452R", ylim = c(0, 1), 
     frame.plot = FALSE, yaxs = "i", col = cols["GISAID"], pch = 16, main = "Proportion de Delta et identification de L452R, 
...
legend("bottomright", col = cols[c("GISAID2", "GISAID",  "criblage")], legend = c("Delta dans les séquences GISAID", "L452R dans les séquences GISAID", "L452R dans le criblage"), pch = c(17, 16, 15), bty = "n")
...
plot(L452R.byweek$week, -L452R.byweek$n, axes = FALSE, type = "h", ylab = "Nb de séqs", xlab = "")
vals <- pretty(seq(0, max(L452R.byweek$n), by = 2000))
axis(2, at = -vals, labels = vals, lwd = 0, lwd.ticks = 1)
axis(4, at = -vals, labels = vals, lwd = 0, lwd.ticks = 1)
#mtext(side = 3, text = "Nombre de séquences")
```
...
Que se passe-t-il en semaine 24-25, pourquoi un tel écart ? Etait-ce un focus sur certaines regions ? 
